...
import com.crane.answer.constants.RedisConstants;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    @Override
    public UserResp userLogin(UserLoginRequest request, HttpServletRequest httpRequest) {
        ...
        //同端互斥登录
        StpUtil.login(user.getId(), DeviceUtils.getRequestDevice(httpRequest));
        // 获取当前配置
    ...
    public boolean addUserSignIn(long userId) {
        ...
        String signInRedisKey = RedisConstants.getUserSignInRedisKey(now.getYear(), userId);
        RBitSet bitSet = redissonClient.getBitSet(signInRedisKey);
        //一年中的第几天
    ...
    public List<Integer> getUserSignInRecord(long userId, Integer year) {
        ...
        RBitSet bitSet = redissonClient.getBitSet(RedisConstants.getUserSignInRedisKey(year, userId));
        List<Integer> dayList = new ArrayList<>();
        BitSet bitSetRecord = bitSet.asBitSet();
        //找到为1的位置
        int index = bitSetRecord.nextSetBit(0);
        while (index >= 0) {
            dayList.add(index);
            index = bitSetRecord.nextSetBit(index + 1);
        }
        return dayList;
    }
